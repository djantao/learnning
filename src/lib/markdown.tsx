export function renderMarkdown(md: string): string {
  ...
  html = html.replace(/```(\w+)?\n([\s\S]*?)```/g, (_, lang, code) => {
    ...
    return `<div class="my-2 rounded-md bg-muted/80 p-2 sm:p-3 overflow-x-auto -mx-2 sm:mx-0">${langLabel}<pre class="text-xs font-mono whitespace-pre"><code>${code.trim()}</code></pre></div>`
  })
